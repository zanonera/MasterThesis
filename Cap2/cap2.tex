...
As radiation-induced faults remain a major challenge for \glspl{fpga} in aerospace and mission-critical systems, numerous studies have proposed mitigation strategies that balance reliability, performance, and resource efficiency. This section presents a synthesis of the most relevant contributions in the literature. Each work discussed here provides valuable information on how different design and mitigation approaches can enhance the robustness of the implementation.
...
\textcite{Balbino2022} numerically validated the efficiency of the Soft-\gls{tmr} redundancy method to enhance communication reliability in a \gls{fpga}-based satellite payload. The study focused on \gls{cots} components and the concept of \gls{sdr} that operates in environments susceptible to \glspl{seu}. An \gls{hdl}-based \gls{seu} emulator and a testbench applying \gls{tmr} to a Quadrature Phase-Shift Keying (QPSK) receiver were developed to evaluate performance. The results demonstrated that \gls{tmr} improved the Bit Error Rate (BER) under emulated \glspl{seu}, though at the cost of a threefold increase in resource utilization and power consumption, providing a clear trade-off reference for designing fault-tolerant communication systems in space applications.

\textcite{Yaman2022} designed a fault-tolerant \gls{riscv}-based processor for space applications. The study applied \gls{cgtmr} to the open-source PicoRV32 core by triplicating the processor and adding word-level voters on the memory bus, without altering the internal IP structure. This voter scheme enhanced error detection by exposing multi-module faults that conventional bitwise voting might mask. A dedicated \gls{tmr} controller monitored the redundancy status, reported software failures and enabled automatic recovery. Implemented on a Xilinx Artix-7 \gls{fpga}, the design demonstrated improved fault tolerance with moderate hardware overhead, offering a practical approach to hardening \gls{cots} cores for low-cost space missions.

\textcite{Wubs2023} evaluated fault-tolerant design techniques for \glspl{fpga} in space-based \gls{dsp} systems, addressing radiation effects such as \glspl{seu}, \glspl{set}, and \glspl{sefi}. The study, applied to the Astrophysical Lunar Observatory (ALO) concept, compared \gls{ms}, \gls{dmr}, \gls{tmr}, \gls{rrtmr} in \gls{fir} and \gls{fft} blocks. The \gls{rrtmr} technique employs one module operating at full precision and two redundant copies running at reduced resolution, in contrast to conventional \gls{tmr}, which replicates three identical full precision modules. The results showed that \gls{tmr} with scrubbing achieved near-complete fault masking, but at the cost of increased resource use, while \gls{rrtmr} offered a more balanced trade-off between protection and hardware efficiency. The work quantified the reliability-cost relationships for radiation-tolerant \gls{fpga} designs in space applications.
...
& Emulation (\acrshort{fi} with \acrshort{semip}) 
& Evaluates \acrshort{rrtmr} and \acrshort{tmr}
...
& Simulation 
& Added \acrshort{secded} \acrshort{ecc} to PicoRV32
...
& Emulation and Physical 
& Evaluates Linux capable \acrshort{riscv} 32 bits 
& No \acrshort{dpr} \\
\\
...
& Emulation (\acrshort{fi} with \acrshort{semip}) \& Simulation with Mutants 
& Low downtime, redundancy \& adaptive repair \& heterogeneous computing support (\gls{soc})
...
From the reviewed literature, some open challenges have received limited investigation. Most existing studies focus on isolated mitigation strategies or rely on proprietary or closed implementations that hinder reproducibility. Another relevant survey was conducted by \textcite{Siecha2025_TDC_SEUEmulation}, which highlights that studies combining multiple \gls{seu} mitigation strategies remain relatively scarce. \textcite{Liu2024_MicroelectronicsJ} presents the recent advances on \gls{fpga} reliability, it concludes that a particularly promising direction for addressing reliability challenges in high-radiation environments is the development of hybrid fault-tolerance methods that combine the strengths of \gls{tmr} and \gls{ms}. The authors also emphasize the need for more robust platforms based on \gls{hw}/\gls{sw} co-design to support more efficient, comprehensive, and accurate reliability evaluation of complex \glspl{fpga} designs.

This work is motivated by the need for a comprehensive and open methodology capable of integrating fault-masking and self-repair mechanisms within a single platform.
...
All scripts implemented and developed design files are openly available in the \href{https://github.com/zanonera/}{GitHub repository} of the project, encouraging reproducibility, transparency, and future extensions by the research community.
