...
Numerous methods have been proposed to enhance the resilience of \gls{fpga} systems. Redundancy-based approaches such as \gls{nmr}, presented in the most common form by \gls{tmr}, memory scrubbing, and configuration repair are among the most widely used. This section surveys existing techniques, comparing their error coverage, resource overhead, and suitability for different applications.
...
To evaluate correctness, the system output is compared against the expected output of a fault-free version, commonly referred to as the \textit{golden system} or \textit{golden run}. 
...
\begin{itemize}
    ...
    or by accessing internal configuration ports (e.g., \textit{\gls{icap}} or \textit{\gls{jtag}}) to emulate SEUs. 
...
\caption{Comparison of \acrshort{fpga} fault-tolerance approaches in related works.} \label{tab:related_works} \\
\hline
\textbf{Reference} 
