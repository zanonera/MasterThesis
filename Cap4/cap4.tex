...
Figure~\ref{fig:fir-dmr} illustrates the proposed architecture used to evaluate the \gls{dmr} design in this study. The design consists of two \glspl{fir} filters and an output voter, implemented to assess the effects of \gls{fi} on the \gls{cram} of the \gls{fpga}.
...
Figure~\ref{fig:dut_fir_bird_view} illustrates the implemented design of the \gls{dmr} architecture using the Vivado tool. The \gls{semip} is instantiate together with the wrapper that constitutes of both \gls{fir} filters and the \gls{ps}. 
...
Figure~\ref{fig:detail_fir} presentes the \gls{dmr} architecture in detail. It is possible to visualize the \gls{rom} with the data to be inputted in the \gls{fir}. The outputs of the designe are checked with the "checker" block and the resullt of the comparsion is sent through serial communication to be evaluated by the \gls{fi} computer.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Cap4/detail_fir.png}
    \caption{Block Diagram, in detail, of the \gls{fir} implementation in Vivado. The \acrshort{dmr} is presented and the circuit to detect if the modules has the correct outputs.}
...
As illustrated in Figure~\ref{error-injection-diagram}, the mitigation core employed in this study is based on the \gls{semip} and AMD/Xilinx primitives that enable readback and correction of the \gls{fpga}'s configuration memory (\gls{cram}). Since the device includes a \gls{ps}, it is utilized both to configure the bitstream and to serve as the \gls{sw} platform for system control. The \gls{dpr} region is employed to dynamically switch functionality within the \gls{pl}, in this case, between a multiplier and an adder module.
