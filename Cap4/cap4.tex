...
Another configuration name for both architectures, was to disable the usage of DSP48 from the \gls{fpga}.
...
The mapping between the \gls{ebd} file and the configuration memory enables the identification of configuration bits that are essential to the implemented design. This mapping provides a practical foundation for correlating the logical elements defined during synthesis with their physical representation in the \gls{fpga} configuration frames, thereby allowing precise targeting of configuration bits during fault injection experiments.
...
The testing strategy is presented to demonstrate how the design was verified at simulation level, emulation level, and on the physical \gls{fpga} hardware. Metrics such as fault coverage, error latency, and resource usage were systematically measured to evaluate the effectiveness of the proposed solution.
