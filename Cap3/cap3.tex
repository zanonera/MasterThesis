...
\caption{Structure of a \gls{clb} in an AMD7-series \gls{fpga}. Reproduced from \textcite{AMD_UG474}}\label{fpga-arch-clb-slice-amd}
\end{figure}
...
\caption{\gls{fpga} Structure. Reproduced from \textcite{AMD_UG474}}\label{fpga-column-asmbl}
\end{figure}
...
Figure~\ref{fpga-simple-bitstream} illustrates, very simplistically, the structure of the bitstream, where the header and footer are represented in the beggining and ending of the structure, the sync word and then the \gls{far} that points to one frame \cite{Xilinx_UG470_7Series_Config}.

Before any configuration packet is processed by the \gls{fpga}, the configuration logic must first detect a specific synchronization pattern. For Xilinx devices, this synchronization word—commonly referred to as the Sync Word—has the hexadecimal value \texttt{0xAA995566}. It allows the configuration logic to align data at a 32-bit word boundary, ensuring correct interpretation of subsequent configuration packets. In parallel configuration modes, successful detection of the bus width is required prior to recognizing the synchronization word \cite{f4pga_prjxray}.
...
Figure~\ref{fpga-seu-and}(a) illustrates one scenario in which a 2-input \gls{lut} is configured by the \gls{cram} bits to describe an $AND$ gate and the \gls{lut} inputs are connected to the inputs $A$ and $B$ by an interconnection switch.
...
\caption{\gls{fpga} Structure. Adapted from \textcite{Mousavi2023_MTTR_FPGA_Scrubbing}}\label{fpga-seu-and}
\end{figure}
