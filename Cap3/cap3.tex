...
As illustrated in Figure~\ref{fpga-column-asmbl}, depending on the device family, the \gls{fpga} architecture may include specialized hardware resources. The different domains represent the various device families, each optimized for specific applications and offering distinct architectural features~\cite{AMD_UG474}. The figure also shows that AMD’s 7-series devices are entirely based on the columnar layout provided by the \gls{asmbl} architecture.
...
\gls{ms} was discussed as a preventive mechanism that periodically restores the integrity of the \gls{fpga} bitstream. Although effective in avoiding error accumulation, scrubbing alone cannot guarantee functional recovery from faults affecting more than one bit per frame at a given time, such as in a \gls{mbu} or in memories such as \gls{bram}. This reinforces the need for a hybrid approach that integrates scrubbing with redundancy and dynamic reconfiguration.
