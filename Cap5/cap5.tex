...
After the successful implementation of scrubbing using \gls{semip} with \gls{dmr} \gls{fir} filters, the next design iteration incorporated \gls{dpr} into the set of functionalities of the system. To fully exploit the resources of \gls{soc}, a \gls{axi} controlled multiplier/adder modules were developed. These hardware functions can be selected through a user menu and dynamically reconfigured within \gls{fpga}.
...
The stalling is caused by the \gls{axi} bus, which operates based on a strict \textit{valid/ready} handshake between the master (e.g., ARM \gls{ps}) and the slave (e.g., custom IP core in the \gls{pl}). If the logic in the \gls{pl} becomes corrupted due to a bit flip—for instance, in the AXI interface registers, finite state machines, or control logic of the reconfigurable region—the \texttt{READY} or \texttt{VALID} signals may become stuck in an asserted or deasserted state. Since the AXI protocol does not implement any form of timeout, when the downstream device fails to complete the transaction, the ARM processor waits indefinitely for a response, effectively freezing software execution.
...
Total injected bits & 21,935 \\
Critical bits detected: Multiplication error & 7 \\
Critical bits detected: AXI bus lock & 16000 \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:mult-list-reprogram} show the completion of \gls{fi} all essentials bits of the \gls{pr} region.
